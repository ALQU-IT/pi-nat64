...
8. Deploys the Flask web UI as a systemd service over HTTPS (port 443, self-signed cert; port 80 redirects to HTTPS)
...
or `https://192.168.50.1`. The UI is served over HTTPS with a self-signed certificate, so your browser will show a one-time warning — that's expected on a local device. Plain `http://` is redirected to `https://`.

The installer generates a **random admin password and prints it once** at the end of the install — save it. You can change it anytime in Settings.
...
- **HTTPS with a self-signed certificate** — the admin password and session cookie are never sent in cleartext. `SESSION_COOKIE_SECURE` and HSTS are enabled when TLS is active; port 80 only issues a redirect to HTTPS.

> Ports 80 and 443 are dropped on the internet-facing `eth0` by the installer, so the UI is reachable only from the AP segment. The TLS certificate is self-signed, so expect a one-time browser warning.
